...
describe("preserveMarksPlugin", () => {
    it("preserves marks when pressing Enter at the end of a marked text", () => {
        ...
        newTr = newTr.setSelection(TextSelection.create(newTr.doc, newTr.selection.from));

        const plugin = preserveMarksPlugin();
        const newState = state.apply(newTr);
        const appendedTr = plugin.spec.appendTransaction?.([newTr], state, newState);
        if (appendedTr) {
    ...
    it("does not preserve marks if not at the start of a new textblock", () => {
        ...
        const plugin = preserveMarksPlugin();
        const appendedTr = plugin.spec.appendTransaction?.([tr], state, state.apply(tr));
